...
<body>
  ...
  <script>
    const devices = [
    { id: '66431b966443b9bfe2b91836', name: 'FIRE SENSOR', status: 'Not Connected' },
    { id: '66431caf202b1b3e1f5900bc', name: 'SMOKE SENSOR', status: 'Not Connected' },
    { id: 3, name: 'TEMPERATURE SENSOR', status: 'Not Connected' },
    { id: 4, name: 'LIGHT SENSOR', status: 'Not Connected' }
  ];

  function createDeviceElement(device) {
    const deviceDiv = document.createElement('div');
    deviceDiv.classList.add('device-div', 'row');

    const deviceNameParagraph = document.createElement('p');
    deviceNameParagraph.classList.add('col', 'font-size-2', 'text-truncate');
    deviceNameParagraph.textContent = device.name;

    const connectButton = document.createElement('button');
    connectButton.classList.add('col-auto', 'btn', 'btn-primary');
    connectButton.textContent = 'Connect';

    connectButton.addEventListener('click', async () => {
      try {
        const response = await fetch('SINRIC_PRO_API_ENDPOINT/connect', {
          method: 'POST',
          headers: {
        ...
        if (response.ok) {
          device.status = 'Device connected successfully';
          statusParagraph.textContent = device.status;
        } else {
          console.error('Failed to connect device:', response.statusText);
          device.status = 'Failed to connect device';
          statusParagraph.textContent = device.status;
        }
      } catch (error) {
        console.error('Error connecting device:', error.message);
        device.status = 'Error connecting device';
        statusParagraph.textContent = device.status;
      }
    });

    const statusParagraph = document.createElement('p');
    statusParagraph.classList.add('col', 'font-size-2', 'text-truncate');
    statusParagraph.textContent = device.status;

    deviceDiv.appendChild(deviceNameParagraph);
  ...
  function addDevicesToContentBody() {
    const contentBody = document.querySelector('.content-body');

    devices.forEach(device => {
      const deviceElement = createDeviceElement(device);
      contentBody.appendChild(deviceElement);
    });
  }

  addDevicesToContentBody();
  </script>
...
<!-- app key - 7a051ff9-6a26-4302-9f03-dae6e59c2b2f
app secret key- 89f41034-d44f-4a22-8da6-bd1fbf5c277d-817e07fd-f7e0-4581-a7b5-2216b15a9167 -->



<!-- 7a051ff9-6a26-4302-9f03-dae6e59c2b2f
89f41034-d44f-4a22-8da6-bd1fbf5c277d-817e07fd-f7e0-4581-a7b5-2216b15a9167 -->